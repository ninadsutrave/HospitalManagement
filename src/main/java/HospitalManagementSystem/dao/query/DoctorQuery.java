...
public class DoctorQuery {
  ...
  public static final String GET_DOCTOR_BY_SPECIALISATION = "SELECT d.name as name, d.specialisation_id, d.years_of_experience, d.shift_start_time, d.shift_end_time, d.is_active, s.name as specialisation FROM doctor d INNER JOIN specialisation s WHERE d.specialisation_id = s.id AND specialisation_id = ? AND is_active = 1";
  public static final String GET_DOCTOR_SHIFT = "SELECT shift_shart, shift_end from doctor where id = ?";
